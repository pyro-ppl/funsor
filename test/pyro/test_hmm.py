...
@pytest.mark.xfail(reason="DiscreteHMM.funsor_dist is undefined")
@pytest.mark.parametrize("state_dim", [2, 3])
@pytest.mark.parametrize("init_shape,trans_shape,obs_shape", DISCRETE_HMM_SHAPES, ids=str)
def test_discrete_categorical_sample_shape(init_shape, trans_shape, obs_shape, state_dim):
    obs_dim = 4
    init_logits = torch.randn(init_shape + (state_dim,))
    trans_logits = torch.randn(trans_shape + (state_dim, state_dim))
    obs_logits = torch.randn(obs_shape + (state_dim, obs_dim))
    ...
    d = DiscreteHMM(init_logits, trans_logits, obs_dist)
    samples = d.sample()
    assert samples.shape == batch_shape + obs_dim


GAUSSIAN_SCHEMA = "init_shape,trans_mat_shape,trans_mvn_shape,obs_mat_shape,obs_mvn_shape"
GAUSSIAN_SHAPES = [
    ((), (), (), (), ()),
    ((), (6,), (), (), ()),
    ((), (), (6,), (), ()),
...
def test_gaussian_hmm_sample(init_shape, trans_mat_shape, trans_mvn_shape,
    ...
    expected_samples = expected_dist.sample()
    actual_samples = actual_dist.sample()
    assert actual_samples.shape == expected_samples.shape
    # TODO check moments
