...
def test_constraints(backend, jit):
    ...
    def model():
        ...
        scales = pyro.param(
            "scales", torch.randn(3).exp(), constraint=constraints.positive
        )
...
def test_elbo_enumerate_plates_1(backend):
    ...
    with pyro_backend(backend):
        pyro.param(
            "probs_a", torch.tensor([0.45, 0.55]), constraint=constraints.simplex
        )
        pyro.param(
            "probs_b",
        ...
        pyro.param(
            "probs_c", torch.tensor([0.75, 0.25]), constraint=constraints.simplex
        )
        pyro.param(
            "probs_d",
        ...
        def auto_model():
            ...
            with pyro.plate("c_axis", 3, dim=-1):
                c = pyro.sample(
                    "c", dist.Categorical(probs_c), infer={"enumerate": "parallel"}
                )
        ...
        def hand_model():
            ...
            for i in range(2):
                ...
                pyro.sample(
                    "b_{}".format(i), dist.Categorical(probs_b[a]), obs=b_data[i]
                )
            for j in range(3):
                c = pyro.sample(
                    "c_{}".format(j),
                ...
                pyro.sample(
                    "d_{}".format(j), dist.Categorical(probs_d[c]), obs=d_data[j]
                )
    ...
    with pyro_backend(backend):
        ...
        def hand_model(data):
            ...
            for i in range(2):
                ...
                pyro.sample(
                    "obs_{}".format(i), dist.Categorical(probs_e[d]), obs=data[i]
                )
...
@pytest.mark.parametrize(
    "exact",
    [True, xfail_param(False, reason="mixed sampling and exact not implemented yet")],
...
def test_gaussian_probit_hmm_smoke(exact, jit):
    def model(data):
        ...
        init_scale = pyro.param(
            "init_scale", 1e-2 * torch.eye(D), constraint=constraints.lower_cholesky
        )
        ...
        noise = pyro.param(
            "noise", 1e-2 * torch.eye(D), constraint=constraints.lower_cholesky
        )
