...
def test_tensor_shape(sample_inputs, batch_inputs, event_inputs):
    ...
    rng_key = subkey = (
        None if get_backend() == "torch" else np.array([0, 0], dtype=np.uint32)
    )
...
def test_transformed_gaussian_shape(sample_inputs, batch_inputs, event_inputs):
    ...
    x = x(
        **{
            name + "_": Variable(name, domain).log()
...
def test_tensor_distribution(event_inputs, batch_inputs, test_grad):
    ...
    if test_grad:
        ...
        assert_close(
            diff_grad, ops.new_zeros(diff_grad, diff_grad.shape), atol=0.1, rtol=None
        )
    else:
        _, mq = diff_fn(p_data)
...
def test_gaussian_distribution(event_inputs, batch_inputs):
    ...
    for k1, d1 in event_inputs.items():
        ...
        # Check first moments.
        assert_close(
            Integrate(q, x, q_vars), Integrate(p, x, p_vars), atol=0.5, rtol=0.2
        )
        for k2, d2 in event_inputs.items():
            ...
            assert_close(
                Integrate(q, x * y, q_vars), Integrate(p, x * y, p_vars), atol=1e-2
            )
