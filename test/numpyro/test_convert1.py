...
from funsor.testing import assert_close, randn, random_mvn, random_tensor
...
def test_matrix_and_mvn_to_funsor(batch_shape, event_shape, x_size, y_size):
    ...
    y_pred = (x[..., None, :] @ matrix).squeeze(-2)
...
def test_matrix_and_mvn_to_funsor_diag(batch_shape, x_size, y_size):
    ...
    scale = ops.exp(randn(batch_shape + (y_size,)))
    ...
    scale_tril = scale[..., None, :] * ops.new_eye(scale, scale.shape)
