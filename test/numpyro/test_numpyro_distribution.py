...
def test_categorical_log_prob(sample_shape, batch_shape, cardinality):
    ...
    logits -= ops.logsumexp(logits, dim=-1)[..., None]
    ...
    value = expected.sample(get_rng_key(), sample_shape)
...
def test_categorical_sample(sample_shape, batch_shape, cardinality):
    ...
    actual_sample = actual.sample(get_rng_key(), sample_shape)
    expected_sample = expected.sample(get_rng_key(), sample_shape)
