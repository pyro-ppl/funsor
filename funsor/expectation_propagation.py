...
from funsor.terms import Funsor, eager, lazy, moment_matching, normalize, reflect, to_funsor  # noqa: F401
...
def ep_generic_contraction_no_adjoint(red_op, bin_op, reduced_vars, terms):
    ...
    for step in range(10):  # fixpoint iteration
        ...
        with interpretation(moment_matching):
            for i, (term, approx) in enumerate(zip(terms, list(approxs))):
                ...
                approxs[i] = bin_op(full_approx, w).reduce(
                    red_op, step_reduced_vars.union(w.inputs) - frozenset(term.inputs))
    ...
    approx = reduce(bin_op, approxs)
    step_reduced_vars = reduced_vars.intersection(approx.inputs)
    result = reduce(bin_op, scalar_terms + [
        bin_op(approx, w).reduce(red_op, step_reduced_vars.union(w.inputs)) for w in ws])
    return result
