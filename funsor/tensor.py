...
def eager_argmax(op, arg, reduced_vars):
    ...
    batch_inputs = OrderedDict(
        (k, d) for k, d in arg.inputs.items() if k not in reduced_vars
    )
    event_inputs = OrderedDict(
        (k, d) for k, d in arg.inputs.items() if k in reduced_vars
    )
    be_inputs = batch_inputs.copy()
    be_inputs.update(event_inputs)
...
def eager_argsample(op, arg, reduced_vars):
    ...
    unscaled_result = arg.unscaled_sample(
        reduced_vars, sample_inputs, rng_key=op.rng_key
    )
